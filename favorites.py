...
def _category_for_item(item):
    ...
    if dbtype in ("song", "album", "artist") or mimetype.startswith("audio/"):
        return "music"
    if dbtype in ("channel", "recording", "episode") and ("pvr" in path or "tv" in path):
        return "tvserver"
...
def _selected_target(item):
    ...
    # Kodi may expose the original plugin target as FileNameAndPath for plugin items.
    if not target.startswith(("plugin://", "pvr://", "http://", "https://", "smb://", "nfs://", "file://")):
        candidate = item.getProperty("originalpath") or item.getProperty("OriginalPath")
        if candidate:
            target = candidate
    return target
...
def promote(item=None):
    ...
    # If the item was already promoted, update it instead of creating a duplicate.
    existing = next((i for i in data["items"] if i.get("target") == target), None)
    if existing:
        existing["folder"] = folder_id
        existing["label"] = title
        existing["thumbnail"] = item.getArt("thumb") or existing.get("thumbnail", "")
        existing["fanart"] = item.getArt("fanart") or existing.get("fanart", "")
        existing["target"] = target
        existing["is_folder"] = bool(item.isFolder())
        _save(data)
        xbmcgui.Dialog().notification("Simple Favourites", "Favorit aktualisiert")
    else:
        data["items"].append({
            "id": _new_id("fav"),
            "folder": folder_id,
...
def _plugin_url(action, **params):
    ...
    return "plugin://%s/%s?%s" % (ADDON_ID, action, query)
...
def _folder_context(data, folder):
    result = []
    if not folder.get("builtin"):
        result.append(("Ordner umbenennen", _run(_plugin_url("rename_folder", folder=folder["id"]))))
        result.append(("Ordner löschen", _run(_plugin_url("delete_folder", folder=folder["id"]))))
...
def _item_context(item):
    result = [
        ("Favorit entfernen", _run(_plugin_url("remove_item", item=item["id"]))),
...
def root():
    ...
    for key, name, icon in CATEGORIES:
        ...
        url = _plugin_url("folder", folder=key)
        _add_item(name, url, icon, is_folder=True, context=_folder_context(data, folder))
...
def folder(folder_id):
    data = _load()
    folder_data = data["folders"].get(folder_id)
    if not folder_data:
        root()
        return

    for child in sorted(_folder_children(data, folder_id), key=lambda x: x.get("name", "").lower()):
        _add_item(child["name"], _plugin_url("folder", folder=child["id"]),
                  child.get("thumbnail", ""), child.get("fanart", ""), True,
                  _folder_context(data, child))

    for item in sorted(_items_for(data, folder_id), key=lambda x: x.get("label", "").lower()):
        target = item.get("target", "")
        # The original URL is retained verbatim in the database. This wrapper only
        # launches it; it does not parse, truncate or reconstruct plugin parameters.
        _add_item(item.get("label", "Favorit"), target,
                  item.get("thumbnail", ""), item.get("fanart", ""),
                  bool(item.get("is_folder")), _item_context(item))
...
def new_folder(parent):
    ...
    if not name:
        return
    if any(f.get("parent") == parent and f.get("name", "").casefold() == name.casefold()
...
def delete_folder(folder_id):
    ...
    if not xbmcgui.Dialog().yesno("Ordner löschen", "Ordner und seine Favoriten löschen?", "", folder.get("name", "")):
        return
    children = [f["id"] for f in data["folders"].values() if f.get("parent") == folder_id]
    doomed = set(children + [folder_id])
    # recursively collect nested folders
    changed = True
    while changed:
        changed = False
        for f in data["folders"].values():
...
def move_item(item_id):
    ...
    if not item:
        return
    folders = sorted(data["folders"].values(), key=lambda f: (str(f.get("parent")), f.get("name", "").lower()))
    labels = [f.get("name", "") for f in folders]
    idx = xbmcgui.Dialog().select("Favorit verschieben", labels)
    if idx < 0:
        return
...
def dispatch():
    if len(sys.argv) < 3 or not sys.argv[2]:
        root()
        return
    query = urllib.parse.parse_qs(sys.argv[2][1:] if sys.argv[2].startswith("?") else sys.argv[2])
